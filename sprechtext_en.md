...
## Slide 2
1.) The main goal was to modify A. woodii so that it can convert 2,3-butanediol into 2-butanol – a valuable bio-based solvent.

2.) Interestingly, A. woodii naturally produces MEK, but doesn’t reduce it further to 2-butanol.
Since the gene for a suitable reductase (pduQ) is present, we wanted to find out whether low expression is the bottleneck.

Context
2-butanol is a promising alternative to fossil-based solvents and fuels.
It has a higher energy density than ethanol, but microbial production is still rare.
Our work is also coupled to CO₂ fixation through the Wood-Ljungdahl pathway – making it a sustainable approach.

## Slide 3 - A. woodii
A. woodii is an anaerobic, acetogenic bacterium that fixes CO₂ using the Wood–Ljungdahl pathway.
It naturally produces acetyl-CoA, but making 2-butanol is quite a metabolic challenge for it.
Luckily, it grows really well under anaerobic conditions and is fairly robust.
It’s also surprisingly easy to modify genetically—and since it doesn’t make any toxic byproducts, it’s safe and pleasant to work with in the lab.
